...
<head>
    ...
    <title>ドラマ一括登録 - MyPlatform</title>
    ...
    <style>
        ...
        .dr-theme-border { border-color: var(--dr-theme); }
        .dr-theme-ring:focus { --tw-ring-color: var(--dr-theme); }
    </style>
    <style>
...
<body class="flex h-screen overflow-hidden text-slate-800 <?= $bodyBgClass ?>"<?= $bodyStyle ? ' style="' . htmlspecialchars($bodyStyle) . '"' : '' ?>>
    ...
    <main class="flex-1 flex flex-col min-w-0 relative">
        <header class="h-16 bg-white/80 backdrop-blur-md border-b <?= $headerBorder ?> flex items-center justify-between px-6 shrink-0 sticky top-0 z-10">
            <div class="flex items-center gap-3">
                ...
                <a href="/drama/list.php" class="flex items-center gap-2 text-slate-500 hover:text-slate-700 transition">
                    <i class="fa-solid fa-arrow-left"></i>
                    <span class="text-sm font-bold">ドラマリスト</span>
                </a>
        ...
        <div class="flex-1 overflow-y-auto">
            <div class="max-w-5xl mx-auto px-6 md:px-12 py-6 space-y-6">
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-700">
                    ...
                    TMDB APIキーが設定されていません。<code class="bg-amber-100 px-1 rounded">private/.env</code> に <code class="bg-amber-100 px-1 rounded">TMDB_API_KEY=xxx</code> を追加してください。<br>
                    仮登録のみ利用可能です。
                </div>
                <?php endif; ?>
                ...
                <div id="step1" class="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                    <h2 class="text-base font-bold text-slate-800 mb-1">
                        <span class="inline-flex items-center justify-center w-6 h-6 rounded-full dr-theme-btn text-white text-xs font-bold mr-2">1</span>
                        ドラマタイトルを入力
                    </h2>
                    <p class="text-xs text-slate-400 mb-4 ml-8">1行に1タイトルずつ入力してください</p>

                    <textarea id="titleInput" rows="10" placeholder="SUITS／スーツ&#10;Breaking Bad&#10;逃げるは恥だが役に立つ&#10;Game of Thrones&#10;..."
                              class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[var(--dr-theme)] focus:border-transparent resize-y font-mono leading-relaxed"></textarea>
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-4 gap-3">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button onclick="DrBulkImport.startSearch()" class="px-5 py-2.5 dr-theme-btn text-white text-sm font-bold rounded-lg shadow-sm transition" id="searchBtn">
                ...
                <div id="step2" class="bg-white rounded-xl border border-slate-100 shadow-sm p-6 hidden">
                    <h2 class="text-base font-bold text-slate-800 mb-3">
                        <span class="inline-flex items-center justify-center w-6 h-6 rounded-full dr-theme-btn text-white text-xs font-bold mr-2">2</span>
                        TMDB検索中...
                    </h2>
                    <div class="flex items-center gap-4 mb-3">
                        <div class="flex-1 bg-slate-100 rounded-full h-3 overflow-hidden">
                            <div id="progressBar" class="progress-bar h-full rounded-full" style="width: 0%; background-color: var(--dr-theme);"></div>
                        </div>
                ...
                <div id="step3" class="hidden space-y-4">
                    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
                        <div class="flex items-center justify-between mb-4">
                            <h2 class="text-base font-bold text-slate-800">
                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full dr-theme-btn text-white text-xs font-bold mr-2">3</span>
                                検索結果プレビュー
                            </h2>
                    ...
                    <div class="flex items-center justify-between">
                        <button onclick="DrBulkImport.reset()" class="px-4 py-2.5 border border-slate-200 text-slate-500 text-sm font-bold rounded-lg hover:bg-slate-50 transition">
                            <i class="fa-solid fa-arrow-rotate-left mr-1"></i>やり直す</button>
                        <button onclick="DrBulkImport.registerAll()" id="registerBtn" class="px-6 py-2.5 dr-theme-btn text-white text-sm font-bold rounded-lg shadow-sm transition">
                ...
                <div id="step4" class="bg-white rounded-xl border border-slate-100 shadow-sm p-8 text-center hidden">
                    ...
                    <div class="flex items-center justify-center gap-3">
                        <a href="/drama/list.php" class="px-5 py-2.5 dr-theme-btn text-white text-sm font-bold rounded-lg shadow-sm transition">
                            <i class="fa-solid fa-list mr-1"></i>ドラマリストへ
                        </a>
                        <button onclick="DrBulkImport.reset()" class="px-5 py-2.5 border border-slate-200 text-slate-500 text-sm font-bold rounded-lg hover:bg-slate-50 transition">
                            続けて登録
                        </button>
                    </div>
                </div>
    ...
    <script>
        ...
        const DrBulkImport = {
            ...
            async startSearch() {
                ...
                for (let i = 0; i < titles.length; i++) {
                    ...
                    if (tmdbConfigured) {
                        try {
                            const res = await fetch(`/drama/api/search.php?q=${encodeURIComponent(title)}`);
                            const json = await res.json();

                            if (json.status === 'success' && json.data.results && json.data.results.length > 0) {
                                const seriesList = json.data.results;
                                ...
                                result.alternatives = seriesList.slice(0, 8);
                            }
                        } catch (e) {
            ...
            renderPreviewRow(r, idx) {
                ...
                if (r.matchType === 'found' && s) {
                    ...
                    poster = imgPath
                        ? `<img src="https://image.tmdb.org/t/p/w92${imgPath}" class="w-12 h-[72px] object-cover rounded-lg shrink-0" loading="lazy">`
                        : `<div class="w-12 h-[72px] poster-placeholder rounded-lg flex items-center justify-center shrink-0"><i class="fa-solid fa-clapperboard text-slate-400 text-sm"></i></div>`;
                    const year = s.first_air_date ? s.first_air_date.substring(0, 4) : '';
                    const rating = s.vote_average ? `<i class="fa-solid fa-star text-amber-400 text-[9px]"></i> ${s.vote_average.toFixed(1)}` : '';
                    info = `
                        <div class="font-bold text-sm text-slate-800 line-clamp-1">${this.esc(s.name || s.title || '')}</div>
                        <div class="text-[11px] text-slate-400 mt-0.5">${year} ${rating}</div>
                        ${s.original_name && s.original_name !== s.name ? `<div class="text-[10px] text-slate-400 line-clamp-1">${this.esc(s.original_name)}</div>` : ''}`;
                } else if (r.matchType === 'existing' && s) {
                    ...
                    const imgPath = s.poster_path || s.backdrop_path || '';
                    poster = imgPath
                        ? `<img src="https://image.tmdb.org/t/p/w92${imgPath}" class="w-12 h-[72px] object-cover rounded-lg shrink-0 opacity-50" loading="lazy">`
                        : `<div class="w-12 h-[72px] poster-placeholder rounded-lg flex items-center justify-center shrink-0 opacity-50"><i class="fa-solid fa-clapperboard text-slate-400 text-sm"></i></div>`;
                    info = `<div class="font-bold text-sm text-slate-400 line-clamp-1">${this.esc(s.name || s.title || '')}</div>
                            <div class="text-[11px] text-slate-400">${s.user_status === 'watched' ? '見た済' : 'リスト登録済'}</div>`;
                } else {
                ...
                <div class="result-row flex items-center gap-3 py-3 ${isExisting ? 'excluded' : ''}" data-idx="${idx}">
                    <label class="shrink-0 flex items-center">
                        <input type="checkbox" class="row-check w-4 h-4 rounded border-slate-300 accent-[var(--dr-theme)]"
                               data-idx="${idx}" ${checked ? 'checked' : ''} ${isExisting ? 'disabled' : ''}
                               onchange="DrBulkImport.toggleRow(${idx}, this.checked)">
                    </label>
                    ${poster}
            ...
            async registerAll() {
                ...
                try {
                    const result = await App.post('/drama/api/bulk_add.php', { items });
            ...
            updateMatch(idx, altIdx) {
                ...
                if (!r || !r.alternatives || !r.alternatives[altIdx]) return;
        ...
        const AltModal = {
            ...
            open(idx) {
                this.currentIdx = idx;
                const r = DrBulkImport.results[idx];
                ...
                if (!r.alternatives || r.alternatives.length === 0) {
                    ...
                    r.alternatives.forEach((s, altIdx) => {
                        const imgPath = s.poster_path || s.backdrop_path || '';
                        ...
                        const year = s.first_air_date ? s.first_air_date.substring(0, 4) : '';
                        const rating = s.vote_average ? `<i class="fa-solid fa-star text-amber-400 text-[9px]"></i> ${s.vote_average.toFixed(1)}` : '';
                        const isSelected = r.match && r.match.id === s.id;
                        const userStatus = s.user_status ? `<span class="text-[10px] text-slate-400">(${s.user_status === 'watched' ? '見た済' : 'リスト登録済'})</span>` : '';

                        container.innerHTML += `
                        <div class="flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-slate-50 transition ${isSelected ? 'ring-2 ring-[var(--dr-theme)] bg-violet-50' : ''}"
                             onclick="AltModal.select(${altIdx})">
                            ${poster}
                            <div class="flex-1 min-w-0">
                                <div class="font-bold text-sm text-slate-800 line-clamp-1">${DrBulkImport.esc(s.name || s.title || '')}</div>
                                <div class="text-[11px] text-slate-400">${year} ${rating} ${userStatus}</div>
                                ${s.original_name && s.original_name !== s.name ? `<div class="text-[10px] text-slate-400 line-clamp-1">${DrBulkImport.esc(s.original_name)}</div>` : ''}
                            </div>
                            ${isSelected ? '<i class="fa-solid fa-check text-[var(--dr-theme)] shrink-0"></i>' : ''}
                        </div>`;
                    });
                    ...
                    <div class="flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-amber-50 transition border border-dashed border-amber-200 mt-2 ${isPlaceholder ? 'ring-2 ring-amber-400 bg-amber-50' : ''}"
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-[11px] text-amber-500">TMDB情報なしで「${DrBulkImport.esc(r.inputTitle)}」として登録</div>
                        </div>
            ...
            select(altIdx) {
                ...
                DrBulkImport.updateMatch(this.currentIdx, altIdx);
                this.close();
            },

            selectPlaceholder() {
                if (this.currentIdx === null) return;
                const r = DrBulkImport.results[this.currentIdx];
                r.match = null;
                r.matchType = 'placeholder';
                DrBulkImport.showPreview();
                this.close();
            }
        };
