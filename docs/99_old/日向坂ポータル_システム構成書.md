...
| is_center | tinyint(1) | センターフラグ（1=センター、ダブルセンター時は該当複数行が1） |
| row_number | tinyint | 1:フロント, 2:2列目, 3:3列目, NULL:不明 |
| position | int(11) | 列内の位置（左端=1、右にカウントアップ） |
...
**フォーメーション構造（position: 左端が1、右にカウントアップ）**:
```
  1  2  3  4  5  6  7  8   ← 3列目（後列）row_number=3
     1  2  3  4  5  6     ← 2列目 row_number=2
        1  2  3  4        ← フロント（1列目）row_number=1（2,3がセンター）
       観客側（手前）
```

**データ例（ダブルセンター: 1列目で position 2,3 がセンター）**:
```sql
-- フロント（1列目）: 4人
(song_id=1, member_id=2, is_center=0, row_number=1, position=1)   -- 左端
(song_id=1, member_id=5, is_center=1, row_number=1, position=2)   -- センター左
(song_id=1, member_id=10, is_center=1, row_number=1, position=3)   -- センター右
(song_id=1, member_id=3, is_center=0, row_number=1, position=4)    -- 右端
```
...
```sql
-- 楽曲登録後、参加メンバーを登録（position: 左端=1、右にカウントアップ）
INSERT INTO hn_song_members (song_id, member_id, is_center, row_number, position) VALUES
-- フロント（1列目）: position 2 がセンター
(1, 5, 1, 1, 2),
(1, 2, 0, 1, 1),
(1, 3, 0, 1, 3),
