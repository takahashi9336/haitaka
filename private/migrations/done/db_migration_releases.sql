...
CREATE TABLE IF NOT EXISTS `hn_songs` (
  ...
  `title_kana` varchar(255) DEFAULT NULL COMMENT 'よみがな',
  `track_type` enum('title','coupling','album_only','bonus','other') DEFAULT 'other' COMMENT '楽曲種別',
  `track_number` int(11) DEFAULT NULL COMMENT 'トラック番号',
  `center_member_id` int(11) DEFAULT NULL COMMENT 'センターメンバーID',
  ...
  FOREIGN KEY (`center_member_id`) REFERENCES `hn_members`(`id`) ON DELETE SET NULL
...
CREATE TABLE IF NOT EXISTS `hn_song_members` (
  ...
  `role` enum('center','fukujin','under','solo','unit','other') DEFAULT 'other' COMMENT '役割',
  `position` int(11) DEFAULT NULL COMMENT 'フォーメーション位置（1=センター）',
