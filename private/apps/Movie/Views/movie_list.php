...
<body class="flex h-screen overflow-hidden text-slate-800 <?= $bodyBgClass ?>"<?= $bodyStyle ? ' style="' . htmlspecialchars($bodyStyle) . '"' : '' ?>>
                        ...
                        <div class="list-row flex items-center gap-4 px-4 py-3 cursor-pointer"
                            ...
                            <div class="shrink-0 flex items-center gap-3">
                                ...
                                <a href="https://www.google.com/search?q=<?= urlencode($mv['title'] . ' 映画') ?>"
                                   ...
                                   class="hidden md:flex w-7 h-7 rounded-md items-center justify-center hover:bg-blue-50 transition opacity-40 hover:opacity-100 shrink-0"
                                   title="Googleで検索">
    ...
    <script>
        ...
        function goDetail(id) {
            const view = document.getElementById('viewList').classList.contains('hidden') ? 'grid' : 'list';
            const params = new URLSearchParams(location.search);
            params.set('view', view);
            sessionStorage.setItem('app:lastListParams:/movie/list.php', '?' + params.toString());
