...
遠征横断で **一覧（集計ビュー）と詳細（`show.php` のタブ：サマリ／参加情報／費用／宿泊／目的地／移動／タイムライン／チェックリスト＋埋め込みマップ等）** が揃い、参加者ロールやマイリスト（持ち物テンプレート）により繰り返し利用できる。遠征は `title` を持つ独立プロジェクトとして先に作成でき、イベント紐付けは後追いで行える。

**印刷用ビュー**: `shiori.php` は別URLとして残置。一覧・詳細の通常UIから「しおり」への導線ボタンは設けない（URL直叩きで利用可能）。
