...
<body class="flex h-screen overflow-hidden text-slate-800 <?= $bodyBgClass ?? '' ?>"<?= ($bodyStyle ?? '') ? ' style="' . htmlspecialchars($bodyStyle) . '"' : '' ?>>
    ...
    <main class="flex-1 flex flex-col min-w-0">
        <header class="h-14 bg-white border-b <?= $headerBorder ?? 'border-slate-100' ?> flex items-center justify-between px-4 shrink-0 sticky top-0 z-20 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <h1 class="font-black text-slate-700 text-lg tracking-tight">動画管理（統合）</h1>
            </div>
